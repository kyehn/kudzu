...
ZEN_CAPTURE = (
    REPO_ROOT / "overlays" / "reasonix" / "opencode" / "POST_zen_v1_chat_completions.json"
)
...
class TestOpenCodeVersionConsistency:
    """opencode 模拟的三源版本必须互锁，漂移即视为模拟失配：
    ...
    三者任一升版都必须同步另两处，否则测试失败。
    """

    def test_fetcher_ua_matches_open_code_version(self) -> None:
        assert MODELS_DEV_USER_AGENT == f"opencode/prod/{OPENCODE_VERSION}/cli"

...
test("getFreeZenModelIds：-free 与 big-pickle，排序", () => {
  assert.deepEqual(getFreeZenModelIds(ZEN), [
    "big-pickle",
    "deepseek-v4-flash-free",
  ]);
});
...
test("lookupModel：原 id 与 . → _ 归一化", () => {
  assert.equal(
    lookupModel(MD.opencode.models, "deepseek-v4-flash-free")?.[0],
    "deepseek-v4-flash-free",
  );
  assert.equal(
    lookupModel(MD.opencode.models, "deepseek.v4.flash_free")?.[0],
    undefined,
  );
  const alt = lookupModel(
    {
      "meta/llama-3.3-70b-instruct":
        MD.nvidia.models["meta/llama-3.3-70b-instruct"],
    },
    "meta/llama-3.3-70b-instruct",
  );
...
test("isChatModel：ctx≥8000 + skip 模式过滤", () => {
  ...
  assert.equal(
    isChatModel(
      "nvidia/embedding-ada-002",
      MD.nvidia.models["nvidia/embedding-ada-002"],
    ),
    false,
  );
  assert.equal(
    isChatModel(
      "meta/llama-3.1-8b-instruct",
      MD.nvidia.models["meta/llama-3.1-8b-instruct"],
    ),
    false,
  );
  assert.equal(
    isChatModel(
      "nvidia/stylegan2-image",
      MD.nvidia.models["nvidia/stylegan2-image"],
    ),
    false,
  );
});
...
test("buildModelProfile：contextWindow/maxTokens/reasoningEfforts/vision", () => {
  const p = buildModelProfile(
    "deepseek-v4-flash-free",
    MD.opencode.models["deepseek-v4-flash-free"],
  );
